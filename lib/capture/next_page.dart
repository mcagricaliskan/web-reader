...
enum NextStrategy {
  ...
  /// A next-ish label inside a chapter-navigation container.
  navContainer,

  /// A next-ish label anywhere on the page.
  labelledControl,

  /// Same series path, chapter number one higher. Corroboration only.
  chapterProgression,
}
...
extension NextStrategyInfo on NextStrategy {
  ...
  NextConfidence get baseConfidence => switch (this) {
    NextStrategy.savedRule => NextConfidence.high,
    ...
    NextStrategy.chapterProgression => NextConfidence.low,
  };

  String get label => switch (this) {
    NextStrategy.savedRule => 'saved rule',
    ...
    NextStrategy.chapterProgression => 'chapter progression',
  };
}
...
const List<String> kNextTextHints = [
  // English
  'next chapter', 'next episode', 'next page', 'next part', 'next', 'continue',
  // Turkish
...
/// Look like "next" but are not chapter navigation.
...
///
/// URL-number incrementing is present only as [NextStrategy.chapterProgression]
/// and is never trusted on its own: fabricating a URL the page never asserted
/// produces phantom chapters, which corrupt the library rather than merely
...
List<NextCandidate> collectNextCandidates(
  ...
  if (ruleHref != null && ruleHref.trim().isNotEmpty) {
    candidates.add(
      NextCandidate(
        href: ruleHref,
        strategy: NextStrategy.savedRule,
  ...
  // Chapter progression: a link on the page whose path matches this series and
  // whose number is exactly one higher. Corroboration for an existing link,
  // never a fabricated URL.
  if (currentUrl != null) {
    final progression = _chapterProgressionCandidates(probe, currentUrl);
...
/// Links that sit in the same series and carry the next chapter number.
List<NextCandidate> _chapterProgressionCandidates(
  PageProbe probe,
  String currentUrl,
) {
  final currentNumber = chapterNumberIn(currentUrl);
  if (currentNumber == null) return const [];
  final series = seriesFingerprint(currentUrl);

  final out = <NextCandidate>[];
  for (final link in probe.links) {
    if (link.href.trim().isEmpty) continue;
    if (seriesFingerprint(link.href) != series) continue;
    final n = chapterNumberIn(link.href);
    if (n == null || n != currentNumber + 1) continue;
    out.add(
      NextCandidate(
        href: link.href,
        strategy: NextStrategy.chapterProgression,
        evidence: 'same series, chapter ${currentNumber + 1}',
      ),
    );
  }
  return out;
...
/// Last number in the final path segment, which is where chapter numbers live
/// on every layout seen so far (`/883-bolum-oku`, `/chapter/101`, `/c/12.5`).
int? chapterNumberIn(String url) {
...
/// links as next-chapter candidates. Only same-host validation stopped them.
...
/// automatic pick walks the job into the wrong series, which is worse than a
/// prompt.
NextPageResult resolveNextPage(
  ...
  if (best.strategy == NextStrategy.savedRule) {
    return NextPageResult(
