...
| **Capture queue** | Decide what to fetch, then fetch it | Adding a capture request queues it · the Library shows `Capture queue · N waiting` · **Start Capture** is the only thing that opens the Browser · queued work survives a restart, still unstarted |
