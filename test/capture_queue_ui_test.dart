...
void main() {
  ...
  Future<void> queueOne(String url) =>
      queue.enqueueCapture(startUrl: url, chapterLimit: 1);

  group('the Library strip', () {
    testWidgets('offers a start when captures are waiting', (tester) async {
      await queueOne('https://x.example/manga/foo/1');
      await show(tester, const LibraryScreen());

      expect(find.byKey(const ValueKey('captureQueueStrip')), findsOneWidget);
      expect(find.text('Capture queue · 1 waiting'), findsOneWidget);
    ...
    testWidgets('stays away when nothing is queued', (tester) async {
      await show(tester, const LibraryScreen());
      expect(find.byKey(const ValueKey('captureQueueStrip')), findsNothing);
      await drain(tester);
    });
    ...
    testWidgets('"Not now" keeps the queue and starts nothing', (tester) async {
      ...
      await tester.tap(find.byKey(const ValueKey('startCaptureButton')));
      await tester.pumpAndSettle();
      expect(find.text('Start queued captures?'), findsOneWidget);
    ...
    testWidgets('confirming asks for the Browser, then runs', (tester) async {
      ...
      expect(executed, ['https://x.example/manga/foo/1']);
      await drain(tester);
    });
  });

  group('the Activity queue section', () {
    testWidgets('waiting captures are their own section', (tester) async {
      await queueOne('https://x.example/manga/foo/1');
      await queueOne('https://x.example/manga/foo/2');
      ...
      expect(find.text('Start 2 captures'), findsOneWidget);
      await drain(tester);
    });
    ...
    testWidgets('each row shows its position and what it will do', (
      ...
      expect(find.textContaining('Capture · 1 chapter'), findsOneWidget);
    ...
    testWidgets('reordering moves a row', (tester) async {
      ...
      expect(rows.first.startUrl, 'https://x.example/manga/foo/2');
      await drain(tester);
    });
    ...
    testWidgets('clearing the queue asks first', (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(find.text('Clear the capture queue?'), findsOneWidget);
