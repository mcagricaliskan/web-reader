...
/// The episode list's progress pie shows the real value, not a bucketed icon.
void main() {
  Widget host(Widget child) => MaterialApp(
    home: Scaffold(body: Center(child: child)),
  );

  ChapterProgressRing ring(double fraction, {bool completed = false}) =>
      ChapterProgressRing(fraction: fraction, completed: completed);
  ...
  testWidgets('0% renders an empty ring, not a filled one', (tester) async {
    ...
    final painter = tester.widget<CustomPaint>(
      find.descendant(
        of: find.byType(ChapterProgressRing),
  ...
  testWidgets('a completed chapter always reads as 100%', (tester) async {
    final handle = tester.ensureSemantics();
    ...
    await tester.pumpWidget(
      host(
        const SizedBox.square(
          dimension: 14,
          child: ChapterProgressRing(fraction: 0.5, completed: false, size: 14),
        ),
      ),
    );
    expect(tester.takeException(), isNull);
    expect(
      tester.getSize(find.byType(ChapterProgressRing)),
      const Size(14, 14),
    );
  });
